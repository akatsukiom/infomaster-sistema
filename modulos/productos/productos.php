...
// Modo depuración para categoría
$categoria_id = null;
if (isset($_GET['categoria'])) {
    $categoria_nombre = $_GET['categoria'];
    $categoria_id = $modelo->obtenerIdCategoriaPorNombre($categoria_nombre);

    if (!$categoria_id) {
        echo "<h2>Error: No se encontró la categoría '" . htmlspecialchars($categoria_nombre) . "'</h2>";
        exit;
    }
}

$productos = $modelo->obtenerTodos($categoria_id);
$titulo = "Todos los productos";

include __DIR__ . '/../../includes/header.php';
?>

<!-- Estilos rápidos para grid -->
<style>
  .productos-grid {
    display: grid;
