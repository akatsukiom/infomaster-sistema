...
if ($esEdicion) {
    ...
    // Asegurarse de que todos los campos existan y estén correctamente asignados
    $producto['categoria_id'] = $datosProducto['categoria_id']; // <- ASEGURAR que exista
    $producto['precio_base'] = $datosProducto['precio_base'] ?? ''; // <- Campo principal de precio
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['crear_categoria'])) {
    ...
    // Detectar si la categoría es de suscripción
    $categoriaNombre = '';
    foreach ($categorias as $cat) {
        if ((int)$cat['id'] === (int)$datos['categoria_id']) {
            $categoriaNombre = strtolower($cat['nombre']);
            break;
        }
    }

    $esSuscripcion = preg_match('/streaming|software|suscripción|plan|premium|netflix|spotify|hbo|max|disney|vix|paramount|iptv|cuenta/i', $categoriaNombre);

    // Validar precios según el tipo de categoría
    if ($esSuscripcion) {
        // Para suscripciones, validar que el precio base sea mayor que 0
        if ($datos['precio_base'] <= 0) {
            $errores[] = 'El precio base debe ser mayor que 0.';
        }
        if ($datos['precio_completo'] <= 0) {
            $errores[] = 'El precio de cuenta completa debe ser mayor que 0.';
        }
        if ($datos['precio_3_meses'] <= 0) {
            $errores[] = 'El precio trimestral debe ser mayor que 0.';
        }
        if ($datos['precio_12_meses'] <= 0) {
            $errores[] = 'El precio anual debe ser mayor que 0.';
        }
    } else {
        // Para productos normales (libros, juegos, cursos), solo validar precio_base
        if ($datos['precio_base'] <= 0) {
            $errores[] = 'El precio base debe ser mayor que 0.';
        }
        // Limpiar precios de suscripción para productos normales
        $datos['precio_completo'] = 0;
        $datos['precio_3_meses'] = 0;
        $datos['precio_12_meses'] = 0;
    }

    // 8.4) Si no hay errores, proceder con el guardado
    if (empty($errores)) {
        try {
            if ($esEdicion) {
                // Actualizar producto existente
                $resultado = $productoModel->actualizar($producto['id'], $datos);
                if ($resultado) {
    ...
    // Si hay errores, mantener los datos en el formulario
    if (!empty($errores)) {
        $producto = array_merge($producto, $datos);
        // Asegurar que la categoría seleccionada se conserve
...
<div class="admin-container">
  ...
  <form method="POST" enctype="multipart/form-data" class="admin-form">
        ...
        <div style="flex-grow: 1;">
          <select id="categoria" name="categoria" required>
            ...
            <?php foreach($categorias as $cat): 
                // Comparación con casting explícito para evitar problemas de tipos
                $sel = ((int)$cat['id'] === (int)$categoriaPrefijada
                     || (int)$cat['id'] === (int)$producto['categoria_id']
                     || (int)$cat['id'] === (int)$categoria_nueva)
                     ? ' selected' : '';
            ?>
    ...
    <div class="admin-form-group">
      <label for="precio_base" class="required">Precio base (<?= MONEDA ?>)</label>
      <div class="input-group">
        <span class="currency-symbol"><?= MONEDA ?></span>
        <input type="number" step="0.01" id="precio_base" name="precio_base" required value="<?= htmlspecialchars($producto['precio_base'], ENT_QUOTES) ?>">
      </div>
      <div class="input-help">Este precio se aplica para productos sin suscripción o como precio mensual base.</div>
    </div>
...
document.addEventListener("DOMContentLoaded", function () {
  // 1. Funcionalidad de categorías y campos dinámicos
  const categoriaSelect = document.getElementById('categoria');
  const camposPlanes = ['precio_completo', 'precio_3_meses', 'precio_12_meses'];
  const categoriasConPlanes = ["streaming", "software", "suscripción", "plan", "premium", "netflix", "spotify", "hbo", "disney", "vix"];

  function toggleCamposPlanes(nombreCategoria) {
    const nombre = nombreCategoria.toLowerCase();
    const debeMostrar = categoriasConPlanes.some(p => nombre.includes(p));

    camposPlanes.forEach(id => {
      const campo = document.getElementById(id);
      const grupo = campo?.closest(".admin-form-group");
  ...
  function detectarCategoriaSeleccionada() {
    const option = categoriaSelect.selectedOptions[0];
    toggleCamposPlanes(option?.textContent || '');
  }

  // 2. Funcionalidad para nueva categoría
  ...
  // 3. Funcionalidad para mostrar nombre de archivo
  ...
  // 4. Ejecutar eventos
  categoriaSelect.addEventListener('change', detectarCategoriaSeleccionada);
  detectarCategoriaSeleccionada(); // Ejecutar al cargar la página
});
</script>
