...
$categorias = [];
if ($rs = $conexion->query("SELECT id, nombre FROM categorias ORDER BY nombre")) {
...
// 7) Procesar formulario para crear nueva categoría
$categoria_nueva = '';
if (isset($_POST['crear_categoria']) && !empty($_POST['nueva_categoria'])) {
    $nombre_categoria = limpiarDato($_POST['nueva_categoria']);
    // Verificar que no exista ya
    $existe = $conexion->query("SELECT id FROM categorias WHERE nombre = '" . 
                 $conexion->real_escape_string($nombre_categoria) . "' LIMIT 1");
    
    if ($existe->num_rows == 0) {
        // No existe, crear
        if ($conexion->query("INSERT INTO categorias (nombre) VALUES ('" . 
                              $conexion->real_escape_string($nombre_categoria) . "')")) {
            $categoria_id = $conexion->insert_id;
            $categorias[] = ['id' => $categoria_id, 'nombre' => $nombre_categoria];
        ...
        $errores[] = 'La categoría ya existe.';
    }
}

// 8) Procesar formulario del producto
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['crear_categoria'])) {
    ...
    $datos = [
        ...
        'tipo_plan'       => 'individual', // Solo plan individual
        'precio_1_mes'    => (float) ($_POST['precio_1_mes'] ?? 0),
    ...
    // El precio base será igual al precio mensual
    $datos['precio'] = $datos['precio_1_mes'];

    // Validaciones
    if ($datos['categoria_id'] <= 0)  $errores[] = 'Selecciona una categoría.';
    if (empty($datos['nombre']))      $errores[] = 'El nombre es obligatorio.';
    if ($datos['precio_1_mes'] <= 0)  $errores[] = 'El precio mensual debe ser > 0.';
    if ($datos['precio_completo'] <= 0) $errores[] = 'El precio de cuenta completa debe ser > 0.';
    ...
    if (!empty($_FILES['imagen']['tmp_name']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowExt)) {
            $errores[] = 'Formato no válido. (jpg, png, webp, gif)';
        } else {
            $uploadDir = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/uploads/productos/';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);

            $slug      = preg_replace('/[^a-z0-9]/i','_', strtolower($datos['nombre']));
            $nuevoName = 'prod_' . substr($slug,0,30) . '_' . time() . '.' . $ext;
            $dest      = $uploadDir . $nuevoName;

            if (!move_uploaded_file($file['tmp_name'], $dest)) {
                $errores[] = 'Error al mover la imagen.';
            } else {
                // Borrar anterior
                if ($imagenExistente && file_exists($_SERVER['DOCUMENT_ROOT'].'/'.$imagenExistente)) {
                    @unlink($_SERVER['DOCUMENT_ROOT'].'/'.$imagenExistente);
                }
                $datos['imagen'] = 'uploads/productos/' . $nuevoName;
            }
        }
    }
    ...
    if (empty($errores)) {
        // Preparo el array con los mismos índices que el modelo espera
        $datosArray = [
            ...
            'precio_1_mes'     => $datos['precio_1_mes'],
            'precio_3_meses'   => $datos['precio_3_meses'],
            'precio_12_meses'  => $datos['precio_12_meses'],
            'precio_completo'  => $datos['precio_completo'],
            'stock'            => $datos['stock'],
            'imagen'           => $datos['imagen']
        ];

        if ($esEdicion) {
            $ok = $productoModel->actualizar($id, $datosArray);
        } else {
            $ok = $productoModel->crear($datosArray);
        }
...
// Añadir CSS personalizado en el head
?>
...
<div class="admin-container">
  <div class="admin-header">
    <h1 class="admin-title"><?= htmlspecialchars($titulo) ?></h1>
  </div>
  ...
  <form method="POST" enctype="multipart/form-data" class="admin-form">
        ...
        <div style="flex-grow: 1;">
          <select id="categoria" name="categoria" required>
            ...
            <?php foreach($categorias as $cat): ?>
              <option value="<?= $cat['id'] ?>" <?= ($cat['id']==$producto['categoria_id'] || $cat['id']==$categoria_nueva)?'selected':''?>>
    ...
    <div class="admin-form-group">
      <div class="precio-card">
        ...
        <div class="input-group">
          ...
          <input type="number" step="0.01" id="precio_1_mes" name="precio_1_mes" required value="<?= $producto['precio_1_mes'] ?>">
        </div>
    ...
    <div class="admin-form-group">
      <div class="precio-card">
        ...
        <div class="input-group">
          <span class="currency-symbol"><?= MONEDA ?></span>
          <input type="number" step="0.01" id="precio_completo" name="precio_completo" required value="<?= $producto['precio_completo'] ?? '' ?>">
        </div>
    ...
    <div class="admin-form-group">
      <div class="precio-card">
        ...
        <div class="input-group">
          <span class="currency-symbol"><?= MONEDA ?></span>
          <input type="number" step="0.01" id="precio_3_meses" name="precio_3_meses" value="<?= $producto['precio_3_meses'] ?>">
        </div>
    ...
    <div class="admin-form-group">
      <div class="precio-card">
        ...
        <div class="input-group">
          <span class="currency-symbol"><?= MONEDA ?></span>
          <input type="number" step="0.01" id="precio_12_meses" name="precio_12_meses" value="<?= $producto['precio_12_meses'] ?>">
        </div>
    ...
    <div class="admin-form-group full-width">
      ...
      <?php if (!empty($producto['imagen'])): ?>
        <div class="img-preview">
          <img src="<?= URL_SITIO . $producto['imagen'] ?>" alt="Imagen actual">
          <div>
            <p>Imagen actual</p>
    ...
    <div class="admin-actions full-width">
      <a href="<?= URL_SITIO ?>admin/productos/listar" class="admin-btn admin-btn-secondary">
